...
public class Portal02Control : MonoBehaviour
{
    ...
    private float circleScale = 1.2f;        // Shader Material CircleScale(Sh_degiþkeni)  degeri Mafth.Lerp takip komutu
    ...
    public float powerInActive = 8f;
    public float powerSpeed = 2.5f;
    public float timeActive = 2.8f; 
    ...
    void Update()
    {
        ...
        if (active)
        {
            ShaderMatControl(circleScale, circleScaleSpeed); // Shader material method control
